...
Claude Code の transcript を分析し、失敗モードごとに改善提案を出すスキル束（プラグイン）。観点ごとに独立した 5 つのスキルから構成され、必要なものだけを呼び出せる:

- **detect-context-rot**: コンテキストロットの始点推定とクロスセッション傾向、A〜E 改善提案
- **detect-token-hotspots**: ターン単位のトークン消費 hot spot 検出と 5 カテゴリの書き換え提案
- **detect-rework-and-violations**: 手戻り（方向修正ループ）と指示違反の検出、5 カテゴリの改善提案、クロスセッション反復追加指示の検知
- **detect-missed-skill-triggers**: スキル/サブエージェントの未トリガ検出と description 改善案生成
- **recommend-bash-allowlist**: auto / bypassPermissions モード下の Bash 実績から allowlist 推薦
