...
allowed-tools: Read, Glob, Grep, "Bash(git remote get-url *)", "Bash(gh pr list *)", "Bash(gh pr view *)", "Bash(gh pr diff *)", "Bash(gh pr merge *)", "Bash(gh release view *)", "Bash(gh search issues *)", "Bash(gh api *)", WebSearch, WebFetch, AskUserQuestion
---
...
- `$ARGUMENTS` が空の場合:
  1. `gh pr list --state open --author "app/dependabot" --json number,title,url` で dependabot のオープン PR を一覧取得
  2. dependabot PR が複数ある場合は AskUserQuestion で対象を選択
  3. dependabot PR が1つもない場合は「対象 PR がありません」と報告して終了

### 3. PR 情報の取得

- `gh pr view <PR番号> --json title,body,baseRefName,headRefName,author` で PR の基本情報を取得
...
- `gh release view <tag> --repo <owner>/<repo> --json tagName,name,publishedAt,body` でリリース情報を取得
  - 最新リリースだけ確認したい場合は `gh release view --repo <owner>/<repo> --json ...` （`<tag>` 省略）
  - GitHub Actions の場合: タグ名は `v{version}` 形式が多い
- リリース日（`publishedAt`）から現在までの経過日数を計算
- **判定**:
  - 7日以上経過: PASS
...
- `gh search issues --repo <owner>/<repo> --state open "<新バージョン> bug regression breaking" --json number,title,url,state` でパッケージリポジトリの Issue を検索
...
- 差分が大きい場合は `gh api "repos/<owner>/<repo>/compare/<old_tag>...<new_tag>"` で新旧タグ間のコミット一覧（`commits[]`）を取得し、各コミットの概要を確認
- 以下の観点で精査:
...
- `gh pr merge <PR番号> --merge` でマージ（マージコミット方式）
