...
ユーザーの transcript JSONL を分析し、**コンテキストロット**（履歴肥大による応答品質の劣化）を検出する特化スキル。本スキルは:
...
- **transcript には機密情報が含まれる可能性がある**。レポート作成時、コマンド例やファイルパスにシークレット類が混入していないか軽くチェックし、疑わしければマスクする（API キー、認証ヘッダ、環境変数ダンプ中の secret 等）
