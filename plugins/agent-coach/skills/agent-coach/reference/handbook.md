...
`detect-missed-skill-triggers` スキル（同プラグイン同梱）を使うと、transcript から利用可能スキル/サブエージェント一覧を復元し、ユーザー入力との突合で未トリガ事例を集約、description 書き換え案（Use when 追加 / キーワード補強 / 競合スキルとの区別 / SKIP 条件）を自動生成できる。観点 5 で 3 件以上の未トリガが見えるプロジェクトでは案内する。
