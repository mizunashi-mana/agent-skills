...
`autodev-create-pr` のインストール前に、PR タイトル・本文をどの言語で記述するかをユーザーに確認する:

- **日本語 (Japanese)**: タイトル・本文ともに日本語（既存の挙動・デフォルト推奨）
- **英語 (English)**: タイトル・本文ともに英語（OSS / 国際チーム向け）
- **その他**: ユーザーが指定する任意の言語

**推奨デフォルト**: README やコミット履歴で使われている主要言語に合わせる。判断が難しい場合は日本語をデフォルトとする。

選択した言語は `autodev-create-pr` テンプレートのカスタマイズに反映する（次節「インストール手順」の手順 2 参照）。
...
2. **プロジェクトに特化できる部分はカスタマイズする**。Step 1〜5 で把握した情報を活用して、以下のような点を調整する:
   ...
   - `autodev-create-pr`:
     - プロジェクトの PR テンプレートパスを明記
     - 「PR タイトル・本文の言語の確認」で選択した言語に合わせて、テンプレート内の `<!-- pr-language: ja -->` 〜 `<!-- /pr-language -->` で囲まれた領域を多言語化する:
       - **マーカーは保持する**（後から言語を変更しやすくするため）
       - 開始マーカーの言語コードを選択言語の ISO 639-1 コード（例: `en`、`zh`、`ko`）に書き換える
       - マーカーで囲まれた本文を選択言語に対応する内容に書き換える
     - 置換例（注意事項マーカー）:

       | 選択言語 | 置換後の本文 |
       | -------- | ------------ |
       | 日本語   | `PR タイトルは日本語で簡潔に（50文字以内推奨）` |
       | 英語     | `PR タイトルは English で簡潔に（50文字以内推奨）` |
       | 中国語   | `PR タイトルは 中文 で簡潔に（50文字以内推奨）` |
     ...
     - 置換例（本文例マーカー、英語選択時）:
       ...
       ## Purpose
       Background and motivation of the change
...
3. ユーザーにインストールしたスキルの一覧と、行ったカスタマイズ内容（選択したレビュー形式と PR 言語を含む）を報告する
