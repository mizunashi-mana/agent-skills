...
2. **プロジェクトに特化できる部分はカスタマイズする**。Step 1〜5 で把握した情報を活用して、以下のような点を調整する:
   ...
   - `autodev-create-pr`:
     ...
     - 「PR タイトル・本文の言語の確認」で選択した言語に合わせて、テンプレート内の以下を書き換える（**日本語以外を選択した場合のみ**）。テンプレート上は `<!-- pr-language: ja -->` 〜 `<!-- /pr-language -->` のコメントで該当箇所をマークしてあるので、その間の本文を選択言語に置換する:
       - 本文例の見出し（`## 目的` / `## 変更概要`）と説明文（`変更の背景・目的` / `主な変更点を箇条書き`）
       - 注意事項の「PR タイトルは日本語で簡潔に（50文字以内推奨）」
     - 置換後はマーカーコメント (`<!-- pr-language: ja -->`、`<!-- /pr-language -->`) 自体をインストール先から削除する（ユーザーが直接編集する想定なので不要）
